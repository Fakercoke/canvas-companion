...
**版本 0.3.0：连接引导与课程工作流。** 它不是自带登录服务的应用，也不增加账号权限。真实接入需要可用连接器/API 工具，或 Agent 能操作的浏览器；普通聊天窗口不一定具备这些能力。
...
旧版曾由无聊天背景 Agent 从公开仓库完成虚构演示，识别日期冲突、缺失评分表及上传未提交。本版增加学校名称起步的接入引导、作业证据与评分检查流程、基于实际使用记录的 AI Declaration；文件检查不能证明真实连接成功。尚未逐一验证各客户端安装、不同学校登录、真实提交或 quiz 操作，不宣称跨学校即装即用。
