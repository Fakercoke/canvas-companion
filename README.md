...
**版本 0.4.0：作业辅助与对话式学习双流程。** 它不是自带登录服务的应用，也不增加账号权限。真实接入需要可用连接器/API 工具，或 Agent 能操作的浏览器；普通聊天窗口不一定具备这些能力。
...
2. **确定学校入口**：只需告诉 AI 学校名字，由它查证官方入口，再指导你在官方页面登录并完成 MFA，或走实际提供的连接器 OAuth 授权。
...
作业辅助之外，也可以直接说：“我没听这几周的课，帮我复习。”Agent 会查找所选课程的 lecture、tutorial、阅读和练习，建立对应关系，读取相关内容后开始讲解，或交付带来源的笔记。对话模式会根据你的回答调整例子和练习；只想要总结时不会强制测验。

录像优先使用可用字幕，转录依赖实际工具与授权；只读取课件时会明确说明。长文献记录实际读过的页码或章节，不把下载成功当成全文读完。备考时优先查课程及学校公开题库，区分已核实真题、样题与 AI 生成练习。

截止日期检查同时核对作业列表、课程总览和相关公告；个人截止、正文日期与入口关闭时间分开记录。提交状态以本人或适用小组的实际记录为依据，无法确认时明确标注。

## 项目包含什么

- `skills/canvas-companion/SKILL.md`：Agent 入口，先接入，再按用户目标执行。
- `references/connect.md`、`keys.md`、`clients.md`：连接路线、密钥解释与配置指导。
- `references/onboarding.md`：新手只报学校名，AI 查入口并执行接入步骤。
- `references/course-reading.md`：整门课、跨课程的读取和覆盖记录。
- `references/learning.md`：课程资料对应、对话学习、录像与真题核验。
- `references/deadlines.md`：跨课程 due 完整性、日期冲突和提交证据检查。
- `references/assignment-workflow.md`：从指定阅读到内容产出、逐项 rubric 检查与交付。
- `references/ai-declaration.md`：根据课程规则及实际 AI 使用记录整理声明。
...
旧版曾由无聊天背景 Agent 从公开仓库完成虚构演示，识别日期冲突、缺失评分表及上传未提交。本版增加对话学习、长文献阅读覆盖记录及截止日期和提交状态核验规则。格式与内部链接检查已通过；新增学习流程尚未完成真实课程端到端测试，文件检查不能证明真实连接成功。尚未逐一验证各客户端安装、不同学校登录、真实提交或 quiz 操作，不宣称跨学校即装即用。
